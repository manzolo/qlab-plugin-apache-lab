#!/usr/bin/env bash
# Common helpers for apache-lab test suite
...
assert_contains() {
    ...
    if echo "$output" | grep -qE "$pattern"; then
        ...
        log_fail "$description (expected pattern: $pattern)"; FAIL_COUNT=$((FAIL_COUNT + 1))
    fi
}
...
assert_not_contains() {
    local description="$1" output="$2" pattern="$3"
    if echo "$output" | grep -qE "$pattern"; then
        log_fail "$description (unexpected pattern: $pattern)"; FAIL_COUNT=$((FAIL_COUNT + 1))
    else
        log_ok "$description"; PASS_COUNT=$((PASS_COUNT + 1))
    fi
}
...
SERVER_PORT="$(_get_port apache-lab)"

if [[ -z "$SERVER_PORT" ]]; then
    echo "ERROR: Cannot find VM port. Is apache-lab running?"
    echo "  Run: qlab run apache-lab"
    exit 1
fi

HTTP_PORT=""
HTTPS_PORT=""
if [[ -f "$STATE_DIR/apache-lab.ports" ]]; then
    HTTP_PORT=$(grep ':80$' "$STATE_DIR/apache-lab.ports" | head -1 | cut -d: -f2)
    HTTPS_PORT=$(grep ':443$' "$STATE_DIR/apache-lab.ports" | head -1 | cut -d: -f2)
fi
...
# ── Cleanup helpers ─────────────────────────────────────────────────
cleanup_apache() {
    log_info "Cleaning up apache test artifacts..."
    ssh_vm "sudo rm -f /etc/apache2/sites-enabled/mysite.conf 2>/dev/null; sudo rm -f /etc/apache2/sites-available/mysite.conf 2>/dev/null; sudo rm -rf /var/www/mysite /var/www/html/secret 2>/dev/null; sudo rm -f /var/www/html/.htaccess /etc/apache2/.htpasswd 2>/dev/null; sudo systemctl reload apache2 2>/dev/null" 2>/dev/null || true
}
