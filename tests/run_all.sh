#!/usr/bin/env bash
# run_all.sh — Run all apache-lab exercise tests in order
#
# Usage:
#   bash tests/run_all.sh              # run all tests
#   bash tests/run_all.sh --skip 03    # skip exercise 3 (SSL)
#
# Prerequisites: apache-lab VM must be running (qlab run apache-lab)
...
echo ""
echo "${BOLD}=========================================${RESET}"
echo "${BOLD}  apache-lab — Automated Test Suite${RESET}"
echo "${BOLD}=========================================${RESET}"
echo ""

log_info "Workspace: $WORKSPACE_DIR"
log_info "Server port: $SERVER_PORT"
log_info "HTTP port: ${HTTP_PORT:-N/A}"
log_info "HTTPS port: ${HTTPS_PORT:-N/A}"
...
run_test() {
    ...
    local files=($script)
    if [[ ! -f "${files[0]}" ]]; then
        log_info "Test file not found: $script"; return
    fi
    if should_skip "$num"; then
        log_info "Skipping exercise $num"; TESTS_SKIPPED=$((TESTS_SKIPPED + 1)); return
    fi
    cleanup_apache >/dev/null 2>&1
    local test_exit=0
    bash "${files[0]}" || test_exit=$?
    TESTS_RUN=$((TESTS_RUN + 1))
    if [[ "$test_exit" -ne 0 ]]; then
        TOTAL_FAIL=$((TOTAL_FAIL + 1)); FAILED_EXERCISES+=("$num")
    else
