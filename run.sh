#!/usr/bin/env bash
# apache-lab run script — boots a VM with Apache web server, SSL/TLS, and virtual hosts
...
echo "    1. Provisioning Apache via cloud-init"
echo "    2. Configuring virtual hosts and .htaccess"
echo "    3. Enabling SSL/TLS with a self-signed certificate"
echo "    4. Testing HTTP/HTTPS from the host via port forwarding"
echo ""
...
echo "    - Install apache2, ssl-cert, and curl packages"
echo "    - Enable mod_ssl and configure virtual hosts"
...
write_files:
  ...
  - path: /etc/motd.raw
    content: |
      \033[1;36m━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\033[0m
        \033[1;32mapache-lab\033[0m — \033[1mApache Web Server Lab\033[0m
      \033[1;36m━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\033[0m
        ...
        \033[1;33mWeb Service Info:\033[0m
          \033[0;32msystemctl status apache2\033[0m      service status
          \033[0;32mcurl localhost\033[0m                 test HTTP
          \033[0;32mcurl -k https://localhost\033[0m      test HTTPS
          \033[0;32m/var/www/html/index.html\033[0m       web root
          \033[0;32m/etc/apache2/sites-available/\033[0m  virtual hosts

        \033[1;33mApache Logs:\033[0m
          \033[0;32m/var/log/apache2/access.log\033[0m
...
runcmd:
  ...
  - a2enmod ssl rewrite
  - openssl req -x509 -nodes -days 365 -newkey rsa:2048 -keyout /etc/ssl/private/apache-selfsigned.key -out /etc/ssl/certs/apache-selfsigned.crt -subj '/CN=localhost/O=apache-lab/C=US'
  - chmod 600 /etc/ssl/private/apache-selfsigned.key
  - chmod 644 /etc/ssl/certs/apache-selfsigned.crt
  - |
    cat > /etc/apache2/sites-available/default.conf <<'VHOST'
    <VirtualHost *:80>
    ...
    VHOST
  - |
    cat > /etc/apache2/sites-available/default-ssl.conf <<'VHOST'
    <VirtualHost *:443>
        ServerName localhost
        DocumentRoot /var/www/html
        SSLEngine on
...
# You can reset the lab by deleting the overlay and re-running.
...
# Step 5: Boot the VM in background with HTTP and HTTPS port forwarding
